...
def get_img(img):
    ...
    for obj in contours:
        ...
        if cv2.contourArea(obj) > 3100:
            ...
            cv2.drawContours(image=image_copy, contours=contours, contourIdx=a, color=(0, 255, 0), thickness=1,
...
img_rgb = img_rgb[0:int(max_loc[1] + h / 2) + 35, 0:451]
...
canny_img = cv2.Canny(img_rgb, 50, 200)
canny_img = cv2.HoughLinesP(canny_img, 1, np.pi/180, 50, minLineLength=10, maxLineGap=250)
for line in canny_img:
    x1, y1, x2, y2 = line[0]
    cv2.line(canny_img, (x1, y1), (x2, y2), (255, 0, 0), 3)
# kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
# canny_img = cv2.dilate(canny_img, (1, 1), iterations=3)
# canny_img = cv2.erode(canny_img, (3, 3), iterations=1)

# img_rgb = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
# canny_img=cv2.adaptiveThreshold(img_rgb,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,9,9)
...
#                               lineType=cv2.LINE_AA)
#get_img(canny_img)
# distance = distance(center_loc, center)
# print(center_loc)
# print(center)
# print(distance)
# time1 = get_time(distance)
# print(time1)
# mouse = Controller()
